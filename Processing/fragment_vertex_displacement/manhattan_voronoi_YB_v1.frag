...
void main() {
    ...
    for(int y = -1; y <= 1; y++)
    {
        for(int x = -1; x <= 1; x++)     
        {
            ...
            // animation
            point = 0.5 + 0.5*sin(u_time*-0.920 + 6.2831*point);            
            ...
            float dist = mix(manhattan(nay, point), manhattan(nay+point, f_st), u_mouse.y/u_resolution.y);
    ...
    // color *= ;
     ...
     color = mix(color, color2*vec3(step(abs(m_dist - m_dist2), 0.204)), 0.284);
