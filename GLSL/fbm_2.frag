...
void main() {
    ...
    vec2 q = vec2(0.);
    q.x = fbm( st + -0.320*u_time + vec2(x_mod,0.0));
    q.y = fbm( st + vec2(-0.320,0.280) + vec2(x_mod,0.0));

    vec2 r = vec2(0.);
    r.x = fbm( st + -0.288*q + vec2(0.230,0.180) + 0.178*u_time + vec2(x_mod,0.0));
    r.y = fbm( st + 3.880*q + vec2(-0.530,0.410) + -0.352*u_time + vec2(x_mod,0.0));
